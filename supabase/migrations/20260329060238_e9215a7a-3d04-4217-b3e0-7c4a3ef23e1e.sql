...
CREATE POLICY "Anyone can read voice samples" ON storage.objects FOR SELECT USING (bucket_id = 'voice-samples');
CREATE POLICY "Authenticated users can upload voice samples" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'voice-samples' AND auth.role() = 'authenticated');
CREATE POLICY "Users can update own voice samples" ON storage.objects FOR UPDATE USING (bucket_id = 'voice-samples' AND auth.role() = 'authenticated');
